...
```
阶段 0 意图确认（对用户）→ 阶段 1 写提示词 → 阶段 2 提示词审核 → 阶段 3 API 出图 → 阶段 4 成图审核 → 阶段 6 每轮交付与台账（每轮结束必交）→ 阶段 5 可编辑矢量版（可选，最终成稿时必问用户）
...
- 🔴 **必须是可编辑矢量**：每个元素是独立对象——PPT 形状、独立 SVG 元素/分组、Illustrator/Inkscape 图层都行。验收标准一句话：**用户能改，你的（宿主 AI）也能改**——后续微调是改对象属性，不是重画
- 🔴 **编辑必须用图层分离的工具**：在 PPT/WPS/Illustrator/Inkscape 里按「图层/组合」组织——每个模块、箭头、标签各自独立成层（PPT 用单独形状或组合、SVG 用 `<g>` 分组、位图软件用独立图层），禁止把所有元素糊成一个扁平对象或直接在扁平层上改像素
- **首选路——比照着重绘**：拿最终确认版图，用本地代码/工具**照着画一遍**——如 python-pptx 生成形状化 PPTX、或写结构化 SVG（每个模块/箭头/标签一个元素），画完与确认版逐项比对、不一致就改，迭代到一致为止。参考做法：<https://github.com/icebird1998/scientific-illustifier>（宿主自行阅读，按本机工具链取舍）
- **退路——描摹**：本机没有可编程矢量工具链、或时间紧时，可以用 vtracer 等把最终版描摹成矢量（`pf export svg` 即此路）。但必须**先核对再交付**：① 数值/标签与最终版一致（数据图数值差不多即可，但刻度对应关系要对）；② 比例、模块对应关系正确；③ 交付时明确告知「描摹产物，文字已变为路径、不可直接编辑，改内容需回到重绘或重出图」——不许把描摹件当「可编辑矢量」宣称
- 宿主连描摹工具都没有时：**明示用户「做不了矢量版」**，交付确认版 PNG 收尾

**交付物**：矢量文件 + 一句「哪些元素可以直接改」（如「每个方框、箭头、文字都是独立形状，可拖动/改字/改色」）；`pf-ledger.md` 补一行，标注本地生成、零 API 消耗。

---

## 🔴 阶段 6：每轮交付与台账（每轮结束必交，用户随时可查）

每次出图 / 优化 / 批处理结束，在用户工作目录建 `promptfigure-out/`，**每轮交付一个新版本文件夹**（v1 → v2 → v3…），协议全文见 `references/deliverables-ledger.md`，五条硬性要求：

1. **本轮清单**：round-vN.md 列出本轮每张图——图种 / 档位 / 版本 / 本机真实路径 / 插入位置
2. **插入位置（两段边界）**：每张图标出插在原文哪两段之间——**上一段最后一句 + 「……」 + 下一段第一句**，用户一眼定位
3. **上下文与引用**：用了哪几段明确到 §¶；上下文只放**首句 + 「……」 + 尾句**；用了本地表格/数据/代码文件一律给**引用链接不内联内容**
4. **图片版本**：文件名带 `-vN`，旧版永不覆盖，随时可回溯对比
5. **两份演进文档**：`paper-with-figures.md`（原文结构 + 已插图，无关段落省略，每次更新同一个）与 `final/paper-final.md`（最终论文版本，附按用户文档格式的可打开副本：Word→docx、LaTeX→tex、WPS→docx）

**最终成稿流程**（用户说「定稿」时）：先提醒核对数据是否更新 → 用户确认后才 premium 定稿 → 定稿后必问矢量图（提醒：本地 AI 重绘、token 贵、速度慢，比赛/数模默认不做；确认做则按元素分层重绘成可编辑矢量，应用按用户偏好）。

🔴 **CLI 优先**：全程走 `pf`（或 REST 直连）沉淀状态，GUI 只留给人看图和点审批——插件的门禁与留痕都挂在 CLI 上。
...
| `references/deliverables-ledger.md` | **每轮交付**：round-vN.md 清单/插入位置/上下文引用、paper-with-figures.md 演进版、final/ 最终论文版本与格式副本、最终成稿流程 |
